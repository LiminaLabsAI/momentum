...
1. **Cerebrio dogfood — Phase 20 `ecosystem upgrade` VALIDATED LIVE (2026-06-20).** Ran `momentum ecosystem upgrade` from `cerebrio-ecosystem` (7 members) on the shipped **v0.22.0** CLI. Result: the clean-tree gate **auto-protected all 6 dirty member repos** (sapience / frontend / py / cli / open-guard / open-shield-python) — the exact blocker that stalled the 2026-06-15 attempt — and `cerebrio-infra` (the one clean repo, a stale v0.13-era install) upgraded **none → 0.22.0**: `.momentum/installed.json` lock file created + **git-trackable (D1 holds in the wild)**, CLAUDE.md Project Extensions preserved, 35-file diff committed on `cerebrio-infra` branch `chore/momentum-upgrade-v0.22.0` (awaiting operator push/PR). **Closes the long-standing "all Phase 17/17.5 validation is synthetic fixtures" gap.** BUG-006 (the prior P1 blocker) was already fixed in v0.20.3. **To finish the fleet:** operator commits/stashes the in-flight work in the 6 dirty repos, then re-runs `momentum ecosystem upgrade` (optionally followed by `/swarm` against a real cross-repo workstream — that path is still synthetic-only). **Observation:** infra's `.gitignore` predates the Phase-20 template, so it lacks the `.momentum/*` + `!.momentum/installed.json` rules — `upgrade` leaves user-owned `.gitignore` untouched (skip-if-exists), so a refresh is operator-discretion.
