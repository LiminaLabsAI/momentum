...
1. **Cerebrio bootstrap** — v0.20.2 is live on npm. **Attempted 2026-06-15, blocked on cerebrio workspace cleanup**. Findings: (a) cerebrio-ecosystem v0.20.2 scaffolding bootstrap PR open at `avinash-singh-io/cerebrio-ecosystem#2` (29 files, 3.4K LOC — first committed scaffolding for that repo; prior v0.13 install was left entirely untracked); (b) the other six member repos have heavy uncommitted in-flight work (sapience 20+, frontend 24+, py 43+, cli 40, open-guard **424**, open-shield-python **176** modified+untracked files) — too dirty for an automated upgrade pass without operator triage; (c) **BUG-006 re-confirmed at scale and bumped P2 → P1**: every installed CLAUDE.md ships the literal `# Project Rules: <Project Name>` first line because `bin/momentum.js` has no template substitution. To resume: operator cleans up the 6 dirty member repos, then we run `momentum upgrade` per repo + `/swarm` against a real cross-repo workstream. Phase 17 + 17.5 validation still uses synthetic fixtures only — live dogfood remains the open gap.
