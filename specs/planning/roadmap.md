...
| 15 | Ecosystem Agent Discoverability | In Progress (target v0.18.0) | ENH-025 close-out — `momentum ecosystem init` auto-writes managed CLAUDE.md + AGENTS.md; action-bearing pointer block in member repos; SessionStart hook prints ecosystem context; `momentum dispatch` CLI surfaces degraded-mode notice upfront; `momentum ecosystem initiative create` CLI ships. Closes the gap between primitives existing and agents reaching for them by default. |
...
| v0.18.0 — Ecosystem Agent Discoverability | 15 | ENH-025 close-out + four sibling discoverability fixes: managed ecosystem CLAUDE.md/AGENTS.md on init; action-bearing pointer block; SessionStart ecosystem-context banner; dispatch CLI upfront degraded-mode notice; `momentum ecosystem initiative create` CLI |
