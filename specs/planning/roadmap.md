...
| v0.34.0 — Lifecycle Cleanup & Default-Branch Hardening | 27 | Default-branch-safe reusable cleanup; push-terminal-first (BUG-025); auto-clean on land + `lanes reconcile` (BUG-026); confirm→verify→clean handshake; tracking-before-release gate; `end_state: open-pr`; `._*`/`.bak` hygiene + `doctor` |
| v1.0.0 — Platform | 29 | MCP server, /specify, /decide, skill authoring; ecosystem Tier 2 |
