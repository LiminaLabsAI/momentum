...
| 21b | Parallel Lanes — Run (Lanes/Board/Queue) | **Complete (v0.24.0 — release parked on approval)** | FEAT-026 lane registry + ambient board + signals (git-common-dir, files-only, no daemon; queue pressure always visible); FEAT-027 merge queue with Rule-14-graded evidence gates; ENH-047 plan-time overlap warnings. Scope committed in `platform-parallel-lanes.md`; seeded brainstorm at start refined by the 21a trial. |
| 21c | Parallel Lanes — Fly (Recursive Waves) | **Complete (v0.25.0 — release parked on approval)** | FEAT-028 dependency annotations (tasks + phases) + one recursive wave planner in core; swarm rewired as top-scale consumer (e2e byte-stable); 3-ideas e2e demo; lane-state contract publication decision. |
...
| 22 | Reach — opencode Adapter | **In Progress (target v0.28.0 — retargeted after OKF took v0.27.0)** | Adapter: opencode — full parity, LIVE-validated in-phase (native commands, plugin hooks, agents, skills:true — a momentum first, swarm spawn via `opencode run --dir`); Cursor (FEAT-007) + Gemini CLI (FEAT-008) stay P1 backlog for a later Reach wave; ENH-009 unblocked by this phase. *(Re-scoped 2026-07-05, operator decision.)* |
...
| v0.24.0 — Parallel Lanes Run | 21b | **Built 2026-07-03 (release parked)** — momentum lanes CLI: registry/board/signals, merge queue with Rule-14-graded gates, overlap warnings; mechanism dogfooded on its own phase |
| v0.25.0 — Parallel Lanes Fly | 21c | **Built 2026-07-03 (release parked)** — one recursive wave engine, swarm rewired byte-stable, dependency annotations + momentum waves, 3-ideas e2e demo; lane-state stays internal (operator decision pending) |
...
| v0.27.0 — Reach: opencode Adapter | 22 | **In progress** — full-parity opencode adapter, live-validated in-phase; skills:true is a momentum first |
| v1.0.0 — Platform | 25 | MCP server, /specify, /decide, skill authoring; ecosystem Tier 2 |
