...
| 29 | Instruction Variation Model (Agent-Neutral Projection) | In Progress (target v0.36.0) | 3-tier instruction sources (principal constants / per-adapter manifest / surface delta); byte-identical neutral spine across every agent + surface; generator auto-discovers adapters (OCP); AGENTS.md composed with an integration section per installed AGENTS.md agent (fixes the same-path destination collision); ecosystem agent-detection fix; BUG-027 verified-fixed + regression guard; TD-009 (opencode fingerprints); ADR-0011. *(Inserted 2026-07-10: Intelligence → 30, Platform → 31.)* |
| 30 | Intelligence | Not Started (target v0.37.0+) | Self-learning hooks (`specs/learnings.md`); retrospective-driven rule evolution; self-healing (recurring failure → ADR proposal); context-window-aware task sizing |
| 31 | Platform | Not Started (target v1.0) | MCP server; `/specify` auto-spec generation; `/decide` (ADR creation); skill/command authoring command; bidirectional spec sync (experimental); ecosystem Tier 2 (federated impact-map, shared rules, deploy-order awareness, multi-repo `/review-code`). *(Dependency-aware task ordering moved to the Parallel Lanes arc — FEAT-028 recursive wave planner.)* |
