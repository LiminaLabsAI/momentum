...
| 25 | Founding Contract (foundation docs authored, not scaffolded) | **In Progress (target v0.32.0)** | ADR-0008 three-state lifecycle (Installed → Founded → Phase loop): init stops shipping the four foundation placeholders; `/start-project` reframed as the founding step; phase commands gate on founded = charter + roadmap exist; `/validate` invariant; upgrade auto-removes provably-untouched placeholder installs. Closes ENH-060. *(Renumber 2026-07-06: Intelligence → 26, Platform → 27.)* |
| 26 | Intelligence | Not Started (target v0.33.0+) | Self-learning hooks (`specs/learnings.md`); retrospective-driven rule evolution; self-healing (recurring failure → ADR proposal); context-window-aware task sizing |
| 27 | Platform | Not Started (target v1.0) | MCP server; `/specify` auto-spec generation; `/decide` (ADR creation); skill/command authoring command; bidirectional spec sync (experimental); ecosystem Tier 2 (federated impact-map, shared rules, deploy-order awareness, multi-repo `/review-code`). *(Dependency-aware task ordering moved to the Parallel Lanes arc — FEAT-028 recursive wave planner.)* |
...
| v1.0.0 — Platform | 27 | MCP server, /specify, /decide, skill authoring; ecosystem Tier 2 |
