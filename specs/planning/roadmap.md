...
| 30e | Ecosystem (Multi-Repo) Team Mode | Not Started (target v0.39.0) | Extends the git-native Team-mode plane to the ecosystem/multi-repo layer (ENH-065, ADR-0015): **remote-URL members**; ecosystem-state (`active-initiative`/initiatives/presence) → per-actor **fragments** shared across machines; **swarm ownership = `refs/momentum/leases/*` CAS as source of truth** (the 30d fence becomes the default when a remote is present; single-machine byte-unchanged, 231 swarm tests green); sample contract reader + "team across repos" docs. Brainstormed 2026-07-10; branch `phase-30e-ecosystem-team-mode`. |
