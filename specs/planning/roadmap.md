...
| 30e | Ecosystem (Multi-Repo) Team Mode | **Complete (v0.39.0)** | Extends the git-native Team-mode plane to the ecosystem/multi-repo layer (ENH-065, ADR-0015): **remote-URL members**; ecosystem-state (`active-initiative`/presence) → per-actor **fragments** shared across machines (conflict-free, attributed); **swarm ownership = `refs/momentum/leases/*` CAS as source of truth** (the 30d fence is default-on when a remote is present, generation-keyed for single-winner + liveness; single-machine byte-unchanged, 236 swarm tests green); sample third-party contract reader + "team across repos" docs + extended demo. Suite 1028/1028. |
