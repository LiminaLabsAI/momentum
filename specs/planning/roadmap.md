...
| 28 | Instruction Consistency | In Progress (target v0.35.0) | Instruction file = pure projection of `specs/` (ADR-0010); `upgrade` syncs all installed agents; ecosystem pointer into every instruction file; shared `specs/project-rules.md` prose surface pointed to by each file; migrate-not-drop Project Extensions; `/complete-phase` release fully config-driven; BUG-027. *(Renumber 2026-07-09: Intelligence → 29, Platform → 30.)* |
| 29 | Intelligence | Not Started (target v0.36.0+) | Self-learning hooks (`specs/learnings.md`); retrospective-driven rule evolution; self-healing (recurring failure → ADR proposal); context-window-aware task sizing |
| 30 | Platform | Not Started (target v1.0) | MCP server; `/specify` auto-spec generation; `/decide` (ADR creation); skill/command authoring command; bidirectional spec sync (experimental); ecosystem Tier 2 (federated impact-map, shared rules, deploy-order awareness, multi-repo `/review-code`). *(Dependency-aware task ordering moved to the Parallel Lanes arc — FEAT-028 recursive wave planner.)* |
...
| v0.35.0 — Instruction Consistency | 28 | Instruction files project `specs/` (ADR-0010); upgrade-all-agents; ecosystem pointer everywhere; `specs/project-rules.md`; config-driven release; BUG-027 |
| v1.0.0 — Platform | 30 | MCP server, /specify, /decide, skill authoring; ecosystem Tier 2 |
