...
| ENH-018 | Tarball-shape test — assert published npm artifact contains expected paths | P1 | open | phase-7b | Surfaced by BUG-002 (v0.7.0 npm tarball was missing adapter overlay files even though the local 24-test suite passed). A test that runs `npm pack --dry-run` (or unpacks the actual tarball) and asserts a list of expected paths would have caught the broken `files` glob before publish. Should run as part of `npm test` and/or as a pre-publish check. |
...
| FEAT-015 | Adapter Contract v3 — runtime metadata for multi-agent compatibility | P1 | in-progress | phase-7b | Adapters declare display name, destinations, primary instruction file, config/hook files, and capability flags. Core remains generic; agent-specific capabilities stay isolated in adapters. |
| FEAT-016 | Adapter: Codex (`AGENTS.md`, `.codex/hooks.json`, command recipes) | P1 | in-progress | phase-7b | First non-Claude adapter. Installs Codex primary instructions, Codex hook config, generic specs/scripts, and Codex-owned command recipes via `momentum init/upgrade --agent codex`. |
| FEAT-017 | Cross-agent install/upgrade regression coverage | P1 | in-progress | phase-7b | Tests prove Claude Code output remains unchanged while Codex install/upgrade works. Includes dynamic available-agent behavior and overlay conflict protection. |
