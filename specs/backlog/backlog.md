...
| TD-002 | `--coding-agent` flag missing from npx CLI | P2 | open | phase-3 | install.sh has `--coding-agent <name>`; bin/momentum.js hardcodes claude-code; needed when Cursor/Gemini adapters land |
...
| ENH-006 | `brainstorm-project.md` missing Group Execution Pattern documentation | P2 | open | phase-3 | brainstorm-phase.md has the full pattern; brainstorm-project.md only says "group-based task breakdown" — Phase 0 plans generated by /brainstorm-project won't have correct structure. |
