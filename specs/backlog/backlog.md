...
| ENH-018 | Tarball-shape test — assert published npm artifact contains expected paths | P1 | open | phase-7 | Surfaced by BUG-002 (v0.7.0 npm tarball was missing adapter overlay files even though the local 24-test suite passed). A test that runs `npm pack --dry-run` (or unpacks the actual tarball) and asserts a list of expected paths would have caught the broken `files` glob before publish. Should run as part of `npm test` and/or as a pre-publish check. |
