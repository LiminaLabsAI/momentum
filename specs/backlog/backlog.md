...
| ENH-023 | Adapter `subagents` capability — inconsistent declaration type | P3 | open | phase-11 | Surfaced during Phase 10 Group 4 capability audit. Claude Code and Antigravity declare `subagents: true`; Codex declares `subagents: 'codex-specific; not used by momentum v0.9.0'`. Phase 11 orchestration code will treat truthy/string as "supported with caveats" but a uniform boolean shape is cleaner. Tracked in `core/adapter-capabilities.md`. |
| ENH-024 | Adapter `skills` / `browser` / `computerUse` — string-vs-boolean inconsistency | P3 | open | phase-11 | Surfaced during Phase 10 Group 4 capability audit. Claude Code declares `false`; Codex declares `'future'` sentinel strings. A `false` declaration plus a roadmap entry is clearer than embedding "future" in capability data. Tracked in `core/adapter-capabilities.md`. |
