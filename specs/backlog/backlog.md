...
| BUG-004 | Session-log concurrent-commit race in `session-append.sh` | P1 | open | phase-10 | Surfaced during Phase 9 review. `core/ecosystem/scripts/session-append.sh` appends with plain `>>` and no lock. Two concurrent commits in two different member repos can interleave or lose lines. Phase 9 overview risk row mentioned "lockfile only on contended platforms" but no lockfile was shipped. Fix lands in Phase 10 Workstream 2: `flock -x` per-session-file lock with atomic-rename fallback. Verified via 10-process concurrent stress test. |
...
| ENH-021 | `momentum ecosystem add/remove/status` location-agnostic | P2 | open | phase-10 | Surfaced during Phase 9 review. Today `momentum ecosystem add/remove/status` must be run from the ecosystem root — awkward for users sitting inside a member repo. Phase 10 Workstream 2 fix: walk up via `findRoot()` if no `ecosystem.json` in CWD; accept `--ecosystem <path>` explicit override (highest precedence). Applies symmetrically to all three subcommands. |
| ENH-022 | `MAX_PARENT_WALK = 5` is a magic number — make configurable | P3 | open | phase-10 | Surfaced during Phase 9 review. `findRoot()` walks at most 5 parent directories looking for `ecosystem.json` with no documentation or override. Phase 10 fix: extract `MAX_PARENT_WALK` constant from `core/ecosystem/lib/index.js` into `core/ecosystem/lib/state.js`; honor `MOMENTUM_MAX_PARENT_WALK` env override in both JS (`state.js`) and shell (`session-append.sh`) paths; document in `core/ecosystem/layout.md` "Discovery & limits" section. |
