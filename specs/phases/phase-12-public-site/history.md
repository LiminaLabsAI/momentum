...
### [DECISION] 2026-06-08 — Cross-repo migration: site moves to trymomentum.github.io
Topics: phase-12, migration, trymomentum, cross-repo-deploy, github-org, pages, base-url
Affects-phases: phase-12-public-site
Affects-specs: .github/workflows/deploy-site.yml, site/astro.config.mjs, package.json, README.md, site/src/components/, site/src/content/docs/
Detail: After Phase 12 PR #6 merged and deploy succeeded at `avinash-singh-io.github.io/momentum`, user requested a cleaner brand URL. Surveyed GitHub username availability — `momentum` is taken; `usemomentum`, `momentumkit`, `momentum-kit`, `trymomentum`, `momentumtoolkit`, `specsmomentum`, `momentumspecs` are free. User chose `trymomentum`. **Pattern locked: two-repo cross-deploy** (same as `sachinshelke/ToolsConnector` → `toolsconnector/toolsconnector.github.io`). Source stays at `avinash-singh-io/momentum`; built site pushes to `trymomentum/trymomentum.github.io` (User/Org Pages). Source repo URL, npm metadata, contributor history all preserved. **User actions** (one-time): registered `trymomentum` org, created the target repo, generated a fine-grained PAT scoped only to the target repo (Contents: R/W), added it as `PAGES_DEPLOY_TOKEN` secret on this repo. **Agent actions**: rewrote `.github/workflows/deploy-site.yml` (build here, `git push --force` to target via PAT, `.nojekyll` added); updated `astro.config.mjs` (`site` → `https://trymomentum.github.io`, `base` → `/`); updated `package.json` `homepage`; updated README banner; sed-replaced every `/momentum/<page>/` site path → `/<page>/` across landing, 3 docs pages, and 2 components (skills.md untouched — its 20 `/momentum/` instances are GitHub source URLs, not site paths). Net: 9 files / 58 lines changed. Merged as PR #8. First deploy at trymomentum URL succeeded; live at https://trymomentum.github.io/ within minutes.

---

### [DECISION] 2026-06-08 — Group 5 verification all green; ready for v0.15.0 release
Topics: phase-12, group-5, verification, lighthouse, linkinator, regression, evidence
Affects-phases: phase-12-public-site
Affects-specs: specs/phases/phase-12-public-site/artifacts/lighthouse-landing.json
Detail: Group 5 (Verification) ran against the live `trymomentum.github.io/` site. **Linkinator**: scanned 24 links across all 9 IA pages — 0 broken (skipped github.com / npmjs.com per convention; spot-checks of those URLs separately green). **CLI regression**: `npm test` on Node 20 — 246/246 pass, 0 fail, 0 skip — matches v0.14.0 baseline exactly, confirming the site phase introduced ZERO CLI behavior change. **Lighthouse landing**: Performance 98, Accessibility 96, Best Practices 100, SEO 100 — all four exceed the ≥ 90 acceptance criterion. Lighthouse JSON report saved at `specs/phases/phase-12-public-site/artifacts/lighthouse-landing.json` (87KB). **Mobile + console-error + OG-preview checks** flagged for user-side visual confirmation (structural responsiveness comes from CSS auto-fit grids by construction; OG card visible at <https://www.opengraph.xyz/url/...>). All acceptance criteria from `overview.md` met or exceeded; phase ready for Group 6 release.

---

### [NOTE] 2026-06-08 — Phase 12 retrospective + v0.15.0 release
Topics: phase-12, group-6, release, retrospective, v0.15.0, npm-publish
Affects-phases: phase-12-public-site
Affects-specs: specs/phases/phase-12-public-site/retrospective.md, package.json, specs/status.md, specs/phases/README.md, specs/phases/index.json, specs/changelog/2026-06.md
Detail: Phase 12 closes as v0.15.0. Bookkeeping batch on `chore/phase-12-release-v0.15.0`: `package.json` version → `0.15.0`, `package-lock.json` regenerated, status.md (Current Phase → between phases / Latest Release → v0.15.0 / Phase 12 row added to Completed Phases / Recent Changes appended), phases/README.md (Phase 12 → Complete), phases/index.json (`status: complete`), changelog 2026-06.md entry. `retrospective.md` written per Phase 11 precedent: what worked (single-phase scope held; full identity pass shipped clean; Lighthouse exceeded by 6-10 pts; cross-repo migration completed in-cycle without re-scoping); what could've been better (initial scaffold cost a Node version install — would've been smoother with Node 22 already; MDX style block ban caught us once; AppleDouble files keep biting on the T7 Shield); deviations (URL migration to trymomentum.github.io was unplanned but in-scope as "URL polish before release"). Tag `v0.15.0` after PR merge. `npm publish --access public` runs with explicit user OK per CLAUDE.md Project Extension constraint.
