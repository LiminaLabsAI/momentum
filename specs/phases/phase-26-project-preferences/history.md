...
### [FEATURE] 2026-07-09 — G5: ENH-062 proactive config-drift sync (built in-phase)
Topics: enh-062, config-sync, drift, approval-gated, self-healing
Affects-phases: phase-26-project-preferences
Affects-specs: core/config.js, bin/momentum.js, core/commands/sync-config.md, adapters/*/instructions/surfaces.md, site/src/content/docs/concepts.md, tests/config-sync.test.js, tests/config-library.test.js
Detail: ENH-062 (filed then built in-phase per operator). `momentum config sync [target]` re-infers the project shape (manifests + git remote) and diffs it against `specs/config.md` over the INFERABLE_KEYS (language/framework/publish_target/git_forge). On drift it shows a per-field old→new diff and asks `[a]ll / comma-separated keys / [s]kip`; on approval it merges the chosen keys via `mergeConfigDrift` (reuses `inferConfig`/`writeConfig`/`writeConfigCache`) and refreshes the derived cache. Never auto-mutates — approval-gated, consistent with the trust posture. New core helpers `diffConfig` + `mergeConfigDrift` + `valuesEqual` (order-insensitive list compare). Upgrade's drift warning now points at `momentum config sync` instead of "edit by hand". `/sync-config` recipe template + help text + recipe-table rows in opencode/codex instruction surfaces (source: surfaces.md). Migration case: a founded project with no config.md → sync creates it. 9 new tests (4 library + 5 CLI incl. piped-stdin apply/skip/dry-run/no-specs). Suite 918 → 927 green; OKF 256; site build green. ENH-062 row stays open in backlog (P2) as the tracking record; the mechanism shipped in v0.33.0.
