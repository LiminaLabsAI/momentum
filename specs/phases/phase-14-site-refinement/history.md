...
### [DECISION] 2026-06-08 — Group 4 makes ecosystem ↔ orchestration explicit as state layer + action layer
Topics: phase-14, group-4, state-action-layers, ecosystem, orchestration, framing, new-diagram, cross-links
Affects-phases: phase-14-site-refinement
Affects-specs: site/src/components/diagrams/StateActionLayers.astro, site/src/components/OrchestrationShowcase.astro, site/src/content/docs/ecosystem.mdx, orchestration.md, concepts.md
Detail: Group 4 (Ecosystem ↔ Orchestration clarity) landed. **NEW `StateActionLayers.astro` diagram** — pure SVG, 800×440 viewBox, no animation. Top half = ACTION LAYER with 4 verb cards (scout / dispatch / handoff / continue) in brand-indigo gradient surfaces under monospace eyebrow "ACTION LAYER · ORCHESTRATION." Bottom half = STATE LAYER on accent-tinted surface with 4 chips (ecosystem.json / initiatives/ / sessions/ / pointer blocks) under eyebrow "STATE LAYER · ECOSYSTEM MODE." Dashed arrows with arrowheads connect each verb down to its corresponding state chip. A "reads + writes" pill sits in the gap between layers. Caption beneath summarizes the relationship: "Orchestration is what the agent does; ecosystem mode is what those actions read and write. Together they're the canonical multi-project pattern — each pointless without the other." **`OrchestrationShowcase.astro` restructure** — outer h2 changed from "One agent. Many repos. No context switching." (over-stated) to **"Work across multiple projects."** Section now holds TWO `<article class="subfeature">` blocks clearly labeled with monospace eyebrows "STATE LAYER" + "ACTION LAYER"; the StateActionLayers diagram sits between them. Sub-feature 1 = Ecosystem mode (description + link to `/ecosystem/`). Sub-feature 2 = Orchestration primitives (description) followed by the existing 4-primitive grid. **`ecosystem.mdx` intro rewritten** — leads with "Ecosystem mode is the **state layer** for multi-project work — the durable record that Orchestration primitives (the action layer) read and write" + cross-link to `/orchestration/` + pair-not-alternatives framing. **`orchestration.md` intro rewritten** — mirrors with "Orchestration is the **action layer** for multi-project work" + cross-link to `/ecosystem/` + same pair-not-alternatives framing. **`concepts.md` Ecosystem section** — NEW "How they relate — ecosystem mode + orchestration" subsection explicitly stating "These two are a **pair**, not alternatives" with the state/action explanation + dual cross-links to both deep pages. Verification: 5 `/orchestration/` cross-links on ecosystem page; 4 `/ecosystem/` cross-links on orchestration page; both eyebrow labels render in dist/index.html (StateActionLayers diagram present on landing). Smoke build green.

---

### [DECISION] 2026-06-08 — Group 5 retires Velocity Arc; new logo is concentric arcs + arrow piercing through, two-tone
Topics: phase-14, group-5, logo, brand, mark, wordmark, favicon, og-card, two-tone, velocity-arc-retired
Affects-phases: phase-14-site-refinement
Affects-specs: site/src/assets/logo/mark.svg, wordmark.svg, site/public/favicon.svg, site/src/components/Logo.astro, LogoMark.astro, site/src/assets/og/og-template.svg, site/public/og/default.png
Detail: Group 5 (New logo + identity refresh) landed. **Velocity Arc retired**; replaced by the **concentric-arcs-and-arrow** mark per user-shared reference image. **Mark design**: 3 progressively-larger concentric semicircles opening to the right (radii 8, 14, 20 in a 64×64 viewBox; slate-400 #94A3B8 stroked, stroke-width 2.5, linecap round) suggest "phases stacking into structure" — a literal visualization of momentum's spec-driven discipline. **Curved arrow** piercing up-right through the arcs (quadratic Bezier `M 10 52 Q 30 30 50 14` stroked indigo-600 #4F46E5 width 6, plus a filled triangular arrowhead at `M 44 4 L 60 14 L 50 26 Z`) suggests "momentum cuts through the structure." Two-tone treatment tells the brand story in the mark: structure underneath (slate, quiet), momentum on top (indigo, accent). **6 assets regenerated**: `mark.svg` (64×64), `wordmark.svg` (260×64 mark + Inter Variable semibold "momentum" text in slate-700), `favicon.svg` (with `prefers-color-scheme: dark` media query lifting arcs to slate-300 + arrow to indigo-400 for dark visibility), `Logo.astro` (Astro component using `currentColor` for wordmark text + explicit hex with `[data-theme='dark']` CSS overrides for arcs/arrow), `LogoMark.astro` (mark-only 64×64 variant for tight spaces), `og-template.svg` (1200×630 OG card — mark scaled 8x to ~320px, indigo-400 arrow + slate-300 arcs against the dark slate-900 background; tagline updated to "Spec-driven development for agentic AI"; footer reads "Phases · Decisions · History · Backlog · Multi-project orchestration"). OG card PNG regenerated to 35KB via prebuild script. Smoke build green: 11 pages, wordmark SVG present in dist. Visual verification (header / favicon / OG preview) deferred to Group 7 post-merge.

---

### [DECISION] 2026-06-08 — Group 6 wholesale README rewrite — visual showcase, GitHub-native Mermaid
Topics: phase-14, group-6, readme, github, mermaid, badges, scannable
Affects-phases: phase-14-site-refinement
Affects-specs: README.md
Detail: Group 6 (README rewrite) landed. **Replaced 361-line / 2,222-word README** with a tight 136-line / 820-word visual showcase that mirrors the site's storytelling for GitHub visitors. **Structure**: (1) centered hero with the new wordmark SVG (relative path `site/src/assets/logo/wordmark.svg` so GitHub renders it inline), tagline "Spec-driven discipline for agentic AI," and 3 badges (npm version, MIT license, "Visit site → trymomentum.github.io" custom shield); (2) "What is momentum?" — 3 short paragraphs locking the agentic-AI positioning ("the state layer for agentic AI"); (3) "Quick install" — primary code block + per-adapter variants + ecosystem; (4) "Works with any AI IDE" — 5-row markdown table with status emojis (✅ Shipped / 🛠️ Planned); (5) "What you get" — 6-feature 2-column markdown table with emoji icons; (6) "Single project ↔ Multi-project ecosystem" — Mermaid `flowchart LR` with two subgraphs (single vs ecosystem), brand-tinted styles (GitHub renders Mermaid natively, no static images needed); (7) "Orchestration primitives" — 4 verbs with one-line descriptions + the state/action pair framing; (8) "The 13 autonomous rules" — 3 standouts (Rule 6/8/12); (9) "Why momentum exists" — 2-paragraph thesis ("state that outlives any single session" + discipline-as-differentiator); (10) "Docs" — bullet list of site section links with emojis; (11) Contributing + License. **Drops the jargon** from v0.16.0's README; everything links to the live site for depth. **GitHub-native rendering**: SVG image via relative path; Mermaid block via fenced code; tables in standard markdown; badges via shields.io. No static screenshots; everything reads cleanly in markdown source. Visual verification (Mermaid rendering, SVG inline) deferred to Group 7 post-merge.
