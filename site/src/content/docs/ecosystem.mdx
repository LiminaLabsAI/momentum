...
- **skips a dirty working tree** (commit/stash first, or pass `--force`) so an
  upgrade never lands on top of unrelated uncommitted work;
...
Preview the whole sweep without writing anything:

```bash
momentum ecosystem upgrade --dry-run
```
