...
Ecosystem mode is the **state layer** for multi-project work — the durable
record that **[Orchestration](/orchestration/)** primitives (the action
layer) read and write. Together they form the canonical multi-project
pattern. Ecosystem mode without orchestration is just bookkeeping;
orchestration without ecosystem mode is action with no record. You'll
typically reach for both.
...
of them** while preserving every project's per-project tracking discipline.
...
layer; cross-project coordination lives next door."
...
- You're not yet on a sustained build cadence — ecosystem mode pays off
  most when there's enough cross-project activity for the shared log + initiatives
  to be useful.
...
npx @limina-labs/momentum@latest join ../my-eco
```
...
```bash
# From the ecosystem root (or any member):
npm install -g @limina-labs/momentum@latest   # update the CLI first
momentum ecosystem upgrade
```

Sweeps `momentum upgrade` across every member in one pass — no more `cd`-ing
...
- **skips a dirty working tree** by default, so an upgrade never lands on top of
  unrelated uncommitted work;
- **detects the adapter** from that repo's `.momentum/installed.json` lock file
  (override with `--agent <name>`);
- runs the upgrade and reports the momentum version **before → after**;
...
```bash
momentum ecosystem upgrade --autostash   # stash → upgrade → restore, per repo
momentum ecosystem upgrade --dry-run     # or preview the whole sweep first
```

`--autostash` stashes each dirty repo's changes, upgrades the now-clean tree,
...
:::tip[You don't need the whole fleet clean]
The sweep is per-repo: it upgrades the clean ones and skips (or, with
`--autostash`, handles) the dirty ones independently. You can also just clean
one repo and re-run — each sweep picks up whatever's ready.
:::

:::note[The CLI version bounds the result]
`momentum upgrade` copies files from the *installed* CLI, not from npm — so a
member can only ever be as new as your CLI. Always `npm i -g
@limina-labs/momentum@latest` before a sweep. See the
[FAQ](/faq/#how-do-i-upgrade-an-existing-project) for the full upgrade model.
:::
...
npx @limina-labs/momentum@latest leave
```
...
When a feature genuinely spans multiple repos, create an **initiative file** at
...
```bash
momentum ecosystem initiative create memory-v1 \
  --why "ship persistent memory across backend + SDK + CLI" \
  --repos sapience,sdk,cli \
  --owner limina
```

Or via the slash door (same code path, agent-friendly):

```
/initiative create memory-v1
```

The created file scaffolds frontmatter (id, slug, status, started, owner,
repos) plus the fixed sections below. The new initiative is also set as the
**active initiative** so the SessionStart hook surfaces it on every new agent
...
- The npm package surface is unchanged — `@limina-labs/momentum` ships
  one CLI that handles both modes.
- The 13 autonomous agent rules apply identically; Rule 9 (multi-project doc
  sync) adds the "don't modify docs you don't own" guard for cross-project
  agent sessions.
...
```bash
cd ~/work/platform
npx @limina-labs/momentum@latest init --ecosystem my-stack
```
...
```bash
cd ~/work/sdk
npx @limina-labs/momentum@latest join ../my-stack
